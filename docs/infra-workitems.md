...
- CUA Site bootstrap and SSO: planned through the Bench Command pattern;
  `site_setup.status` and `site_setup.complete` should use native Frappe setup
  APIs; OAuth/user/access commands remain `Unsupported` until Infra
  live-verifies setup status/completion on a real Site
